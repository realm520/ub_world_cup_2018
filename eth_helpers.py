...
def send_eth_rawtransaction_to_myetherapi(data):
    """
...
def get_eth_address_nonce(eth_address):
    ...
    res = call_myetherapi_rpc('eth_getTransactionCount', [eth_address, 'latest'])
...
def eth_signtransaction(transaction_dict, eth_private_key):
    """对以太交易内容进行签名"""
    tx = {}
    for k, v in transaction_dict:
        tx[k] = v
    if tx.get('chainId', None) is None:
        tx['chainId'] = 1
    account = eth_account.Account.privateKeyToAccount(eth_private_key)
    res = account.signTransaction(transaction_dict=transaction_dict)
